...
- (void)viewDidLoad {
    ...
    if(kFBSDK){
        ...
        TWTRLogInButton *logInButton = [TWTRLogInButton buttonWithLogInCompletion:^(TWTRSession *session, NSError *error) {
            if (session) {
                NSLog(@"signed in as userId:%@ ,userName %@", [session userID],[session userName]);
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [client loadUserWithID:[session userID] completion:^(TWTRUser *user, NSError *error) {
                        ...
                        _userHeadUrl = user.profileImageLargeURL;
                    }];
                });
            } else {
    ...
    if(kFBSDK){
        ...
        [[Twitter sharedInstance] logInWithCompletion:^(TWTRSession *session, NSError *error) {
            if (session) {
                ...
                [client loadUserWithID:[session userID] completion:^(TWTRUser *user, NSError *error) {
                    // handle the response or error
                    //formattedScreenName :@app_cdhk
                    //user.name : CDHK_APP
                    //user.screenName : app_cdhk
                    //https://pbs.twimg.com/profile_images/1025264164053245953/qq0I3dqu_reasonably_small.jpg
                    _userHeadUrl = user.profileImageLargeURL;
                }];
...
- (IBAction)shareContent:(id)sender {
    ...
    NSString *shareImageUrl = @"https://www.chinadailyhk.com/attachments/image/164/209/125/400933_112283/400933_112283_956_500_jpg.jpg";
    NSString *shareLinkUrl = @"https://www.chinadailyasia.com/articles/126/230/9/1533261962100.html";
    ...
    if(kFBSDK){
        ...
        if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"fb-messenger-share-api://"]]) {
            ...
            element.defaultAction = detailButton;
        ...
        //设置分享的标题
        [composer setText:shareTitle];
        //设置分享的URL
        [composer setURL:[NSURL URLWithString:shareLinkUrl]];
        //添加图片, 这里分享的图片传值是 UIimage类型
        UIImage *shareImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:shareImageUrl]]];
